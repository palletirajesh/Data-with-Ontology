...
def evaluate_and_update_ontology(user_text, original_sql, edited_sql):
    ...
    if "MISMATCH" not in response:
        try:
            # Strip markdown formatting if the LLM wrapped the JSON in triple backticks
...
if user_input:
    ...
    with col_sql:
        ...
        # In a real setup, this is where you load context from JSON-LD/Markdown
        # and prompt Llama-3 to write the SQL.
        
        with st.spinner("Translating intent to SQL via Llama-3..."):
            # SYSTEM PROMPT INCLUDES THE ALIAS RULE TO PREVENT AMBIGUOUS COLUMNS
            system_prompt = f"""
            ...
            SELECT and WHERE clauses with its corresponding alias.
            
            Write BigQuery SQL for this request: "{user_input}"
            Return ONLY the SQL code, nothing else.
            """
            
            # Fetch the generated SQL from the LLM
            final_sql = call_groq_llm(system_prompt).replace("```sql", "").replace("```", "").strip()
            
        # Ensure we only reset the editor if the user asked a NEW question
        if "last_user_input" not in st.session_state or st.session_state.last_user_input != user_input:
            st.session_state.last_user_input = user_input
            st.session_state.sql_editor_key = final_sql # Bind directly to the text_area key
            st.session_state.original_generated_sql = final_sql
            # Clear previous dataframe
            if "last_df" in st.session_state:
                del st.session_state["last_df"]

        st.subheader("📝 Review and Edit SQL")
        
        # Use an st.form to lock the state and prevent ghost-updates
        ...
        if execute_btn:
            ...
            with st.status("🚀 Running BigQuery Job...", expanded=True) as status:
                try:
                    ...
                    # Check if the user made changes to the SQL
